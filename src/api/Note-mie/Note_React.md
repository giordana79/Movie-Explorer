...
Insieme a HTML e Javascript completa la parte di Front-End.
React è un Framework (libreria Javascript) sviluppata da Meta(Facebook). Serve a costruire delle interfacce utente moderne ed interattive. con questa. libreria flessibile e potente si creeranno delle userinterface iterattive. é dichiarativa cioè si descriive cosa si vuole vedere sullo schermo e React si preoccupa di aggiornare il DOM in modo efficiente. Facilita le operazioni di aggiornamento di una pagina Web.
Perchè usare React? Dispone di componenti riutilizzabili: ogni pezzo dell'interfaccia è un blocco indipendente, mantiene un codice ordinato e lo rende manutentabile.Permette aggiornamenti automatici significa. che quando cambia lo stato React aggiorna solo ciò che serve. Ha un enorme ecosistema di routing, gestione dati, libreria UI già pronte. React è usata ovunque per es. Netflix, Instagram, Airbnb.
...
vediamo di defninire quale argomento da passare alla funzione Saluto:
...
questa funzione prende il nome passato come argomento e lo restituirà come risullato della funzione

4. ## State (useState)

Sono delle variabili React che ricorda tra un render ed un al'ltro, quando cambia l'interfaccia grafica si aggiorna in automatico.
Per es:
...
Mantiene una sorta di rapprentazione interna della user-interface chiamata Virtual DOM.
...
Aprire con VS in my-react-app src/App.jsx e cancellaret utto il contenuto all'interno e si inserisce il seguente:
...
per riavviare si lancia il comando -> npm run dev
...
assets,components(Titolo.jsx),data(projeect.js) e styles(dentro c'è index.html) sono cartelle create da noi su src
